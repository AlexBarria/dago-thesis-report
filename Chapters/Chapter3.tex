...
\captionof{figure}{Serie temporal de la distancia pulgar-índice en una sesión de finger tapping.}
...
Agregación y\\exportación & Tracking y métricas. & CSV, JSON, resumen de sesión. & \texttt{SessionAggregator};\\escritores. \\
\bottomrule
\end{tabular}}
\end{minipage}
\end{center}
...
Las etapas de estimación de pose, seguimiento y segmentación se detallan en la sección~\ref{sec:arquitectura}, mientras que ingestión, preprocesamiento y calibración corresponden al flujo documentado en esta sección.
...
La configuración global se centralizó en la clase \texttt{Settings}, que se carga con prefijo de variables de entorno \texttt{DAGO\_} y valores por defecto versionados en el repositorio. De ese modo se evitó codificar rutas, umbrales de audio o geometría del tablero ChArUco dentro de la lógica. El tipo de ejercicio clínico (\texttt{finger\_tapping}, \texttt{hand\_tremor}, \texttt{nose\_finger}, \texttt{arm\_oscillation}) se propagó a lo largo del pipeline para etiquetar métricas y seleccionar el modelo de pose adecuado.
...
La figura \ref{fig:pipeline-dago} muestra el recorrido completo desde el video crudo hasta la persistencia y las interfaces de consulta. En el diagrama se indica de forma genérica la estimación de pose con MediaPipe. En la implementación, los ejercicios centrados en la mano emplean el modelo \texttt{HandLandmarker} y los que requieren contexto corporal el \texttt{PoseLandmarker} lite, según lo descrito en la sección \ref{sec:modelos-preentrenados}. También se observa la separación entre el procesamiento analítico en CPU, la capa de persistencia relacional y los canales de acceso mediante API y aplicación web. Esa organización condicionó las decisiones de integración desarrolladas en la sección \ref{sec:integracion}.
...
La función \texttt{pose\_model\_for\_exercise} selecciona el landmarker de MediaPipe según el tipo de ejercicio: modo \texttt{HANDS} para \texttt{finger\_tapping} y \texttt{hand\_tremor}, y modo \texttt{HOLISTIC} para \texttt{nose\_finger} y \texttt{arm\_oscillation}. La clase \texttt{PoseEstimator} instancia el modelo correspondiente mediante la API de tareas, con archivos \texttt{.task} en caché local gestionados por \texttt{ModelManager}. En el servicio web se activa el modo de video del landmarker para explotar la continuidad temporal entre cuadros consecutivos.

Las coordenadas de profundidad (\texttt{z}) se almacenan en los esquemas internos pero no intervienen en distancias ni ángulos métricos: la cuantificación operativa permanece en dos dimensiones con escala inferida por ChArUco o calibración manual. En modo \texttt{HANDS} se retiene una sola mano dominante por fotograma; en modo \texttt{HOLISTIC} se registran además hombros, codos y muñecas para visualización.

\subsection{Seguimiento temporal y señales derivadas}

\texttt{KeypointTracker} acumula, por cada fotograma analizado, un registro tabular común con índice temporal, coordenadas normalizadas de los 21 puntos de la mano dominante, distancias derivadas en milímetros cuando existe calibración y, en modo holístico, posiciones de nariz, hombros, codos y muñecas junto con los ángulos de abducción de brazo izquierdo y derecho (\texttt{pose\_left\_arm\_angle}, \texttt{pose\_right\_arm\_angle}). Las detecciones ausentes se registran como valores faltantes y se rellenan hacia adelante hasta un máximo de 5 fotogramas consecutivos para atenuar oclusiones breves. Esa capa de seguimiento es compartida por los cuatro tipos de ejercicio, pero la señal que alimenta las métricas clínicas se elige en función del movimiento evaluado.

En \texttt{finger\_tapping} y \texttt{hand\_tremor} la señal operativa principal es la distancia pulgar--índice \texttt{hand0\_thumb\_index\_dist}. Con factor de escala $s$ milímetros por píxel y coordenadas normalizadas de pulgar e índice $\mathbf{p}_t$ y $\mathbf{q}_t$ en un fotograma de ancho $W$ y alto $H$ píxeles, se define
\begin{equation}
d_t = s \,\sqrt{W^2(p_{t,x}-q_{t,x})^2 + H^2(p_{t,y}-q_{t,y})^2}\,.
\label{eq:thumb-index-dist}
\end{equation}
En \texttt{finger\_tapping}, $d_t$ sustenta los biomarcadores de tapping y la sincronía con el metrónomo. En \texttt{hand\_tremor}, la misma serie (o el desplazamiento de la muñeca derivado de \texttt{hand0\_wrist\_px\_*}) cuantifica la oscilación involuntaria de la mano en reposo, sin detección de ciclos de apertura y cierre.

En \texttt{nose\_finger} se construye la distancia entre la yema del índice y la nariz a partir de los landmarks holísticos (\texttt{pose\_nose\_*} y \texttt{hand0\_lm8\_*}), con la misma conversión métrica $s$. El mínimo de esa distancia durante el alcance resume la precisión del contacto nariz--dedo. En \texttt{arm\_oscillation} la magnitud central es el ángulo de abducción del brazo en el plano de la imagen, calculado en cada fotograma entre el vector hombro--codo y la línea de hombros. Las columnas de muñeca y mano quedan disponibles para supervisión visual, pero las métricas automáticas de ese ejercicio se expresan en grados sobre la serie angular.

\subsection{Métricas de movimiento implementadas}

El módulo de métricas selecciona, según el tipo de ejercicio, la señal temporal y el conjunto de algoritmos aplicables. Las funciones genéricas \texttt{oscillation\_amplitude}, \texttt{tremor\_frequency} y \texttt{movement\_regularity} operan sobre la serie activa de cada ejercicio cuando hay al menos 4 muestras válidas. La frecuencia dominante se estima por transformada rápida de Fourier en la banda
\begin{equation}
...
En \texttt{finger\_tapping}, tras recortar la fase estática y, de forma opcional, la ventana entre el primer y el último tap, se aplican sobre $\{d_t\}$ los biomarcadores de \texttt{compute\_finger\_tapping\_metrics}: tasa de taps, intervalo inter-tap medio y su coeficiente de variación, decremento de amplitud entre picos, índice de fatiga, velocidades medias de apertura y cierre, y el promedio del ángulo en la muñeca (\texttt{thumb\_index\_wrist\_angle\_mean}). La detección de picos usa \texttt{scipy.signal.find\_peaks} con prominencia proporcional al rango de la señal \citep{sibley2021video,acevedo2025visionmd}. \texttt{SessionAggregator} calcula además retardos entre impulsos del metrónomo y mínimos locales de $d_t$ dentro de $\pm 0{,}35$~s (\texttt{delays.csv}).

En \texttt{hand\_tremor}, sobre la serie de la mano en reposo se reportan amplitud de oscilación, frecuencia en la banda \ref{eq:tremor-band} y regularidad del movimiento. No se calculan métricas de tapping ni retardos con metrónomo.

En \texttt{nose\_finger}, se cuantifican la distancia mínima y media índice--nariz, la longitud de la trayectoria de la yema del índice y un índice de precisión del alcance (cociente entre distancia mínima y longitud de trayectoria). Opcionalmente se resume la dispersión de la muñeca durante el gesto.

En \texttt{arm\_oscillation}, se promedian y resumen los ángulos de abducción (\texttt{arm\_angle\_mean}, \texttt{arm\_angle\_variability}), se aplican amplitud, frecuencia y regularidad sobre la serie angular del brazo dominante y se estima la estabilidad postural del tronco (\texttt{postural\_stability}) a partir del desplazamiento del centro de masa definido por hombros y caderas.
...
\begin{itemize}
    \item \textbf{Tapping de dedos y temblor de mano:}
    \begin{itemize}
        \item \textbf{Amplitud de oscilación} (\texttt{oscillation\_amplitude}, mm): rango de $d_t$ o de desplazamiento de muñeca.
        \item \textbf{Frecuencia de temblor} (\texttt{tremor\_frequency}, Hz): pico espectral sobre la señal de mano, banda definida según \ref{eq:tremor-band}.
        \item \textbf{Regularidad del movimiento} (\texttt{movement\_regularity}, adim.): variabilidad y entropía de la oscilación de mano.
    \end{itemize}

    \item \textbf{Tapping de dedos:}
    \begin{itemize}
        \item \textbf{Tasa de taps} (\texttt{tap\_rate}, 1/s): ritmo de taps asociado a bradicinesia.
        \item \textbf{Intervalo inter-tap medio} (\texttt{iti\_mean}, s): tiempo medio entre taps consecutivos.
        \item \textbf{Coeficiente de variación del ITI} (\texttt{iti\_cv}, adim.): consistencia del ritmo de tapping.
        \item \textbf{Decremento de amplitud} (\texttt{amplitude\_decrement}, mm/s): pendiente de amplitud entre picos sucesivos.
        \item \textbf{Índice de fatiga} (\texttt{fatigue\_index}, adim.): amplitud final respecto de la amplitud inicial en la ventana dinámica.
        \item \textbf{Velocidad de apertura en picos} (\texttt{peak\_opening\_velocity}, mm/s): velocidad media al abrir los dedos en los picos de $d_t$.
        \item \textbf{Velocidad de cierre en valles} (\texttt{peak\_closing\_velocity}, mm/s): velocidad media al cerrar los dedos en los valles.
        \item \textbf{Ángulo medio en muñeca} (\texttt{thumb\_index\_wrist\_angle\_mean}, $^{\circ}$): apertura media pulgar--índice en la muñeca.
        \item \textbf{Retardo audio--movimiento} (\texttt{delays}, s): desfase entre el metrónomo y el mínimo de $d_t$, si hay audio disponible.
    \end{itemize}

    \item \textbf{Prueba nariz--dedo:}
    \begin{itemize}
        \item \textbf{Distancia mínima nariz--dedo} (\texttt{nose\_finger\_min\_distance}, mm): mínima distancia entre la yema del índice y la nariz.
        \item \textbf{Distancia media nariz--dedo} (\texttt{nose\_finger\_mean\_distance}, mm): distancia media durante el alcance.
        \item \textbf{Longitud de trayectoria} (\texttt{nose\_finger\_path\_length}, mm): recorrido de la yema del índice.
        \item \textbf{Precisión del alcance} (\texttt{nose\_finger\_accuracy}, adim.): cociente entre distancia mínima y longitud de trayectoria.
    \end{itemize}

    \item \textbf{Oscilación de brazo:}
    \begin{itemize}
        \item \textbf{Ángulo medio de abducción} (\texttt{arm\_angle\_mean}, $^{\circ}$): abducción media del brazo analizado.
        \item \textbf{Variabilidad del ángulo} (\texttt{arm\_angle\_variability}, $^{\circ}$): rango pico a pico del ángulo de abducción.
        \item \textbf{Amplitud de oscilación} (\texttt{oscillation\_amplitude}, $^{\circ}$): oscilación de la serie angular del brazo.
        \item \textbf{Frecuencia de temblor} (\texttt{tremor\_frequency}, Hz): frecuencia dominante del ángulo en banda definida según \ref{eq:tremor-band}.
        \item \textbf{Regularidad del movimiento} (\texttt{movement\_regularity}, adim.): regularidad de la oscilación del brazo.
        \item \textbf{Estabilidad postural} (\texttt{postural\_stability}, adim.): balanceo del tronco, estimado como proxy hombro--cadera.
    \end{itemize}
\end{itemize}
...
\texttt{SessionAggregator} recibe el \texttt{DataFrame} completo de seguimiento (incluida la fase estática cuando corresponde), la lista de métricas calculadas, los impulsos de audio y los metadatos de calibración. Produce un resumen con identificador de video, tipo de ejercicio, tasa de fotogramas nominal, conteo de fotogramas analizados, tabla de métricas y colección de retardos audio--movimiento. Los escritores \texttt{CsvWriter} y \texttt{JsonWriter} materializan ese contenido en disco con convenciones de nombres de columna estables, de modo que las pruebas de integración puedan verificar la existencia y el esquema de \texttt{tracking.csv}, \texttt{metrics.csv} y \texttt{summary.json} tras una ejecución de humo del comando de sesión.

La métrica de amplitud de oscilación combina rango pico a pico, media y desviación estándar de la señal centrada; la regularidad cuantifica tanto la variabilidad relativa como la entropía muestral de incrementos, conforme a la literatura de análisis de complejidad del movimiento en trastornos del movimiento \citep{pecoraro2025computer}. La coherencia entre definiciones matemáticas en código y descripción en la memoria se verificó en el capítulo~\ref{Chapter4} mediante pruebas unitarias sobre señales sintéticas con espectro y picos conocidos.
...
En entorno local, \texttt{docker-compose} levanta los servicios \texttt{db} (\texttt{postgres:16-alpine}), \texttt{api} (imagen multi-etapa con \texttt{Python} 3.12, dependencias instaladas con \texttt{uv} y límite de memoria de 8~GB) y \texttt{frontend} (\texttt{nginx} sirve el build estático y proxifica la API). Un perfil opcional \texttt{cli} permite ejecutar el mismo contenedor en modo batch con el módulo \texttt{pipeline.cli.run\_session}. Los modelos MediaPipe y los videos de muestra se montan como volúmenes para no incluirlos en la imagen y facilitar la actualización.

El punto de entrada del contenedor de API ejecuta migraciones de esquema con \texttt{Alembic} antes de levantar el servidor \texttt{uvicorn}, de modo que un despliegue nuevo aplique automáticamente las revisiones pendientes sobre \texttt{PostgreSQL}. El frontend se construyó en una etapa previa de la imagen multi-etapa (\texttt{Node.js} para compilar TypeScript y \texttt{nginx} para servir archivos estáticos) y el runtime de inferencia se fijó en \texttt{linux/amd64} por compatibilidad con las ruedas binarias de MediaPipe en arquitecturas sin soporte oficial para brazo.
