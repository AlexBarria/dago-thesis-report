...
La figura \ref{fig:pipeline-dago} muestra el flujo desde el video crudo hasta las interfaces de consulta. La estimación de pose emplea \texttt{HandLandmarker} para ejercicios de mano y \texttt{PoseLandmarker} lite para contexto corporal (sección \ref{sec:modelos-preentrenados}). Además, se distingue el procesamiento analítico en CPU, la persistencia relacional y el acceso mediante API y web. Esta arquitectura condicionó las decisiones de integración detalladas en la sección \ref{sec:integracion}.
...
\end{equation}
coherente con el rango de interés clínico para temblor y oscilaciones de extremidad superior \citep{pecoraro2025computer}, para lo cual se emplea la tasa efectiva tras el subsampleo ($f_s = \mathrm{fps}_{\mathrm{video}} / \mathrm{step}$).
