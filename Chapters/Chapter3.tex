...
Las etapas de ingestión, preprocesamiento y calibración corresponden al flujo documentado en esta sección. Mientras que las etapas: estimación de pose, seguimiento y segmentación se detallan en la sección~\ref{sec:arquitectura}.
...
En \texttt{finger\_tapping} la señal operativa principal es la distancia pulgar--índice \texttt{hand0\_thumb\_index\_dist}. Con factor de escala $s$ milímetros por píxel y coordenadas normalizadas de pulgar e índice $\mathbf{p}_t$ y $\mathbf{q}_t$ en un fotograma de ancho $W$ y alto $H$ píxeles, se define
\begin{equation}
...
En \texttt{hand\_tremor}, sobre la serie de la mano en reposo se reportan amplitud de oscilación, frecuencia en la banda definida según \ref{eq:tremor-band} y regularidad del movimiento. No se calculan métricas de tapping ni retardos con metrónomo.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Distancia mínima nariz--dedo} (\texttt{nose\_finger\_min\_distance}, mm): mínima distancia entre la yema del índice de cada mano y la nariz.
        \item \textbf{Distancia media nariz--dedo} (\texttt{nose\_finger\_mean\_distance}, mm): distancia media durante el alcance.
        \item \textbf{Longitud de trayectoria} (\texttt{nose\_finger\_path\_length}, mm): recorrido de la yema del índice de cada mano.
        ...
        \item \textbf{Ángulo medio de abducción} (\texttt{arm\_angle\_mean}, $^{\circ}$): abducción media de cada brazo (métrica secundaria).
        \item \textbf{Variabilidad del ángulo} (\texttt{arm\_angle\_variability}, $^{\circ}$): rango pico a pico del ángulo de abducción (métrica secundaria).
        \item \textbf{Estabilidad postural} (\texttt{postural\_stability}, adim.): balanceo del tronco, estimado como proxy hombro--cadera (métrica secundaria).
    \end{itemize}

    \item \textbf{Apertura y cierre de manos:}
    \begin{itemize}
        \item \textbf{Amplitud de apertura} (\texttt{oscillation\_amplitude}, mm): rango de apertura máxima de cada mano.
        \item \textbf{Frecuencia de ciclos} (\texttt{open\_close\_rate}, 1/s): ritmo de los ciclos voluntarios de apertura y cierre.
        \item \textbf{Regularidad del movimiento} (\texttt{movement\_regularity}, adim.): variabilidad y entropía de los ciclos.
        \item \textbf{Velocidad de apertura en picos} (\texttt{peak\_opening\_velocity}, mm/s): velocidad media al abrir cada mano.
        \item \textbf{Velocidad de cierre en valles} (\texttt{peak\_closing\_velocity}, mm/s): velocidad media al cerrar cada mano.
        \item \textbf{Índice de simetría entre manos} (\texttt{hand\_movement\_symmetry}, adim.): comparación relativa de la amplitud, velocidad y frecuencia de apertura/cierre entre ambas manos para cuantificar la coordinación bilateral.
